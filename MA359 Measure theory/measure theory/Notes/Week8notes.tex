...
%\emph{extra non-examinable bit} We can also prove that there exists a continuous function $g$ such that $g$ coincides with $f$ on $K$ and $\sup|g| = \sup_K |f|$. We do this by considering our approximating sequence $f_n$. We can construct a $g_n$ which coincides with $f_n$ on $K_n$. We do this by considering the construction of $K_n$ in the first part of the proof as $K_n = \bigcup_{k=1}^{N_n} K_{n,k}$ where each $K_{n,k}$ is compact and $f_n$ restricted to $K_{n,k}$ is a constant which we can call $a_{n,k}$. Then Urysohm's lemma allows us to construct a continuous function $\phi_{n,k}$ which is 1 on $K_{n,k}$ and 0 on all the other $K_{n,i}$ for $i \neq k$. Then let us write $h_n = \sum_{k=1}^{N_n} a_{n,k} \phi_{n,k}$, we then take another function $\psi(x) = x$ for $x \in [-1,1]$ and $\psi(x) = -1$ for $x < -1$ and $\psi(x) = 1$ for $x>1$ then $\psi$ is continuous and let $\psi_n(x) = (\sup_Kf) \psi(x/(\sup_K f))$ then set $g_n = \psi_n \circ h_n$ and we have constructed a $g_n$ which coincides with $f_n$ on $K_n$ and is cointinuous. Now using Egoroff's theorem there exists a $\tilde{K}_\epilon$ such that $g_n$ converges uniformly on $\tilde{K}$ 
\end{proof}
...
%Final important application
%\begin{ex}[Fourier series]
%Suppose that $f:[0,2\pi] \rightarrow \mathbb{R}$ is an $L^1$ function. Then we can write $f$ as a Fourier series
%\[ f(x) = \sum_k (a_k \cos(kx) + b_k \sin(kx)), \] where $a_k = \int_0^{2\pi}f(x) \cos(kx)\mathrm{d}x$ and $b_k(x) = \int_0^{2\pi} f(x) \sin(kx) \mathrm{d}x$. Then let $F(x) = \int_0^x f(y)\mathrm{d}y$. One of the original motivations for developing Lebesgue integration was to know when 
%\[ F(x) = \sum_k \left( \frac{a_k}{k}\sin(kx) - \frac{b_k}{k}(\cos(kx)-1)\right). \] That is to say, when can we integrate the Fourier series term by term.
%\[
%F(x) = \int_0^x f(y)\mathrm{d}y = \int_0^x \left( \sum_k (a_k \cos(ky) + b_k \sin(ky)) \right) \mathrm{d}y.
%\] We would like to switch the order of the sum and the integral. Let $\mu$ be the measure on $\mathbb{N}$ which gives $\mu(\{k\}) = 1$ for every $k \in \mathbb{N}$. Then let $g(k,y) = a_k \cos(ky) + b_k \sin(ky)$ then 
%\[ F(x) = \lambda( \mu(g)1_{[0,x]}), \]
%\end{ex}
